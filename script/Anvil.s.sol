...
contract EulerInvariantAnvilScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(EulerInvariantHook).creationCode, abi.encode(address(manager)));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        EulerInvariantHook eulerInvariantHook = new EulerInvariantHook{salt: salt}(manager);
        ...
        vm.startBroadcast();
        testLifecycle(address(eulerInvariantHook), initializeRouter, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
